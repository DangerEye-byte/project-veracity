...
class AuditRequest(BaseModel):
    model_config = ConfigDict(protected_namespaces=()) # Fixes potential 3.14 conflicts
    query: str
    response: str

# Mock 'Ground Truth' for first test
# In Step 2, we will replace this with PDF loader
...
#     relevant_docs = retriever.invoke(data.query)
    
#     # Add relevance threshold — check score if available
#     if not relevant_docs:
#         return {"status": "NEUTRAL", "message": "No relevant documents found to verify against."}

#     context = relevant_docs[0].page_content

#     # Simple keyword overlap check — if query and doc share no meaningful words, skip
#     query_words = set(data.query.lower().split()) - {"the", "a", "an", "is", "in", "of", "to", "was", "what", "when", "how", "which"}
...
#     return {"status": "VERIFIED", "message": "Claim matches ground truth."}


# main.py — only the verify endpoint changes (one line)
@app.post("/verify")
async def verify_flow(data: AuditRequest):
    ...
    query_words = set(data.query.lower().split()) - {
        "the","a","an","is","in","of","to","was","what",
        "when","how","which","are","does","do","tell","me"
    }
    context_words = set(context.lower().split())
    overlap = query_words & context_words
    print(f"DEBUG - Keyword overlap: {overlap}")
    ...
    # Pass query into auditor so it can verify doc relevance
    verdict = audit_response(context, data.response, query=data.query)
    print(f"DEBUG - Verdict: {verdict}")
    ...
    return {"status": verdict, "message": "Claim matches ground truth." if verdict == "VERIFIED" else "Could not verify claim against knowledge base."}
